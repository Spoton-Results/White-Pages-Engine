-- Migration: 0002_startup_schema_adds
-- These were previously running as inline ALTER TABLE statements on every
-- server boot inside runBackgroundStartup(). They are now a proper one-time
-- Drizzle migration. Run once via: npx drizzle-kit migrate
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ── One-time data repair: spotonresults.com pages domain normalization ────────
-- This was running on every boot. Now runs exactly once as a migration.
UPDATE websites
SET
  ...
  settings = (
    ...
    || jsonb_build_object(
         'parentDomain',       'pages.spotonresults.com',
         'publicDomain',       'pages.spotonresults.com',
         'proxyPath',          '',
         'publicBasePath',     '',
         'legacyParentDomain', 'spotonresults.com',
         'legacyProxyPath',    'pages'
       )
  ),
  updated_at = NOW()
WHERE
  lower(domain) = 'spotonresults.com'
  OR lower(domain) = 'pages.spotonresults.com'
  OR lower(COALESCE(settings->>'parentDomain', '')) = 'spotonresults.com'
  OR lower(COALESCE(settings->>'parentDomain', '')) = 'pages.spotonresults.com'
  OR lower(COALESCE(settings->>'publicDomain', '')) = 'spotonresults.com'
  OR lower(COALESCE(settings->>'publicDomain', '')) = 'pages.spotonresults.com'
  OR lower(COALESCE(settings->>'legacyParentDomain', '')) = 'spotonresults.com';
