...
export default function ActionReviewPage() {
  ...
  async function loadJobs(websiteId = selectedWebsiteId) {
    ...
    setLoading(true);
    try {
      const data = await fetchJson<ReviewJob[]>(`/api/websites/${websiteId}/action-review`);
      setJobs(data);
    } catch (e: any) {
    ...
    <DashboardLayout>
      <div className="mx-auto max-w-7xl space-y-6 px-6 py-8">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          <Card><CardContent className="p-4"><Clock3 className="h-5 w-5 text-blue-600" /><p className="mt-3 text-2xl font-bold">{stats.total}</p><p className="text-xs text-gray-500">Total review jobs</p></CardContent></Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Review and approve SEO governance actions before anything destructive is allowed.</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="overflow-hidden rounded-lg border">
              <Table>
                ...
                <TableBody>
                  {jobs.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={7} className="py-10 text-center text-sm text-gray-500">No review jobs found for this website.</TableCell>
                    </TableRow>
                  ) : jobs.map(job => (
                    <TableRow key={job.id}>
                      <TableCell>
                      ...
                      <TableCell>
                        <div className="space-y-1">
                          <div className="font-medium">{job.winnerSlug || "—"}</div>
                          <div className="text-xs text-gray-500">Tier {job.winnerTier ?? "—"}</div>
                        </div>
                      </TableCell>
                      <TableCell>{job.intentCluster || "—"}</TableCell>
                      <TableCell>
                        <Badge className={job.status === "completed" ? "bg-green-100 text-green-800 hover:bg-green-100" : job.status === "failed" ? "bg-red-100 text-red-800 hover:bg-red-100" : "bg-blue-100 text-blue-800 hover:bg-blue-100"}>{job.status}</Badge>
                      </TableCell>
                      <TableCell>
                        <div className="space-y-1 text-xs">
                          <div>{job.requiresConfirmation ? "Requires confirmation" : "No confirmation"}</div>
                          <div>{job.redirectRequiredBeforePrune ? "Redirect required" : "No redirect required"}</div>
                          <div>{job.destructiveActionAllowed ? "Destructive enabled" : "Safe mode"}</div>
                        </div>
                      </TableCell>
                      <TableCell className="text-sm text-gray-500">{new Date(job.createdAt).toLocaleString()}</TableCell>
                      <TableCell>
                        <div className="flex flex-wrap gap-2">
                          <Button size="sm" className="gap-1" disabled={!!busyId} onClick={() => review(job.id, "approved")}><CheckCircle2 className="h-3.5 w-3.5" />{busyId === job.id + "approved" ? "Working..." : "Approve"}</Button>
                          <Button size="sm" variant="outline" className="gap-1" disabled={!!busyId} onClick={() => review(job.id, "needs_changes")}><RefreshCcw className="h-3.5 w-3.5" />Changes</Button>
                          <Button size="sm" variant="destructive" className="gap-1" disabled={!!busyId} onClick={() => review(job.id, "rejected")}><XCircle className="h-3.5 w-3.5" />Reject</Button>
                        </div>
                      </TableCell>
                    </TableRow>
                  ))}
