...
import { AlertTriangle, BarChart3, CheckCircle2, Clock, Database, GitBranch, Layers, Link2, Merge, Play, RefreshCcw, Search, ShieldCheck, Sparkles, Target, TrendingUp } from "lucide-react";
...
interface AppliedAction { action: IntentAction; label: string; detail: string; appliedAt: string; }
...
function actionButtonLabel(action: IntentAction) { if (action === "promote") return "Promote"; if (action === "link") return "Add Links"; if (action === "improve") return "Improve"; if (action === "consolidate") return "Consolidate"; return "Merge"; }
...
function actionLabel(action: IntentAction) { if (action === "promote") return "Promoted"; if (action === "link") return "Links Added"; if (action === "improve") return "Improve Job Queued"; if (action === "consolidate") return "Consolidation Review Queued"; return "Merge Review Queued"; }
...
export default function IntentBuildPage() {
  ...
  useEffect(() => {
    loadWebsites().catch(e => setError(e.message)).finally(() => setLoading(false));
  }, []);

  useEffect(() => {
    if (!selectedWebsiteId) return;
    loadStatusAndReport(selectedWebsiteId).catch(e => setError(e.message));
  }, [selectedWebsiteId]);

  useEffect(() => {
    if (!selectedWebsiteId || status.status !== "running") return;
    const timer = window.setInterval(() => loadStatusAndReport(selectedWebsiteId).catch(e => setError(e.message)), 1500);
    return () => window.clearInterval(timer);
  }, [selectedWebsiteId, status.status]);
    ...
    try {
      ...
      setAppliedActions(current => ({
        ...
        [key]: {
          ...
          detail: result.jobId ? `${owner.canonicalOwner} → job ${result.jobId}` : `${owner.canonicalOwner} updated successfully`,
    ...
    <DashboardLayout>
      <div className="mx-auto max-w-7xl space-y-6 px-6 py-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <div className="flex items-center gap-2"><div className="rounded-lg bg-blue-50 p-2 text-blue-700"><Target className="h-5 w-5" /></div><h1 className="text-2xl font-bold tracking-tight text-gray-900">Intent Ownership Build</h1></div>
            <p className="mt-2 max-w-3xl text-sm text-gray-500">Live canonical-owner report, internal-link actions, promotion controls, and non-destructive review queues.</p>
          </div>
          <div className="flex flex-col gap-3 sm:flex-row">
            <Select value={selectedWebsiteId} onValueChange={setSelectedWebsiteId} disabled={loading || websites.length === 0}>
            ...
            <Button onClick={handleRunBuild} disabled={!selectedWebsiteId || status.status === "running"} data-testid="btn-run-build" className="gap-2"><Play className="h-4 w-4" />{status.status === "running" ? "Running..." : "Run Build"}</Button>
            <Button variant="outline" onClick={handleRefresh} disabled={!selectedWebsiteId || status.status === "running"} data-testid="btn-refresh" className="gap-2"><RefreshCcw className="h-4 w-4" />Refresh</Button>
          </div>
        </div>
