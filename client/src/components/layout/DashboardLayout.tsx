    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-3 mt-auto border-t shrink-0">
        <div className="flex items-center gap-3 px-2 py-1.5">
          ...
          <div className="flex flex-col flex-1 min-w-0"><span className="text-sm font-medium leading-none truncate">{user?.username || "..."}</span><span className="text-xs text-muted-foreground truncate">{user?.role || ""}</span></div>
  ...
  return (
    <div className={stacked ? "flex flex-col gap-2 w-full" : "flex items-center gap-2"} data-testid="agency-client-switcher">
      <Select value={selectedAgencyId ?? "all"} onValueChange={handleAgencyChange}>
        <SelectTrigger className={`h-8 text-xs gap-1 ${stacked ? "w-full" : "w-[160px]"}`} data-testid="select-agency"><Handshake className="size-3 text-muted-foreground shrink-0" /><SelectValue placeholder="All Agencies"><span className="truncate">{selectedAgencyLabel}</span></SelectValue></SelectTrigger>
        <SelectContent><SelectItem value="all">All Agencies</SelectItem>{(agencies as any[]).map((a: any) => (<SelectItem key={a.id} value={a.id}>{a.name}</SelectItem>))}</SelectContent>
      </Select>

      <Select value={selectedAccountId ?? "all"} onValueChange={handleAccountChange}>
        <SelectTrigger className={`h-8 text-xs gap-1 ${stacked ? "w-full" : "w-[160px]"}`} data-testid="select-client"><Building2 className="size-3 text-muted-foreground shrink-0" /><SelectValue placeholder="All Clients"><span className="truncate">{selectedAccountLabel}</span></SelectValue></SelectTrigger>
        <SelectContent><SelectItem value="all">All Clients</SelectItem>{filteredAccounts.map((a: any) => (<SelectItem key={a.id} value={a.id}>{a.name}</SelectItem>))}</SelectContent>
      </Select>
    </div>
  );
}
      ...
      <div className="min-h-screen bg-background flex">
        ...
        <main className="flex-1 flex flex-col min-h-screen overflow-hidden">
          <header className="h-14 border-b bg-background/95 backdrop-blur sticky top-0 z-30 flex items-center justify-between px-4 gap-3">
            ...
            <div className="md:hidden flex items-center gap-2 font-bold text-base"><div className="size-6 rounded bg-primary flex items-center justify-center text-primary-foreground"><Globe className="size-3.5" /></div><span>Nexus</span></div>
            <div className="flex-1 hidden md:block" />
            <div className="flex items-center gap-3"><div className="hidden md:flex"><AgencyClientSwitcher /></div><Button variant="ghost" size="icon" className="text-muted-foreground size-8"><Bell className="size-4" /></Button></div>
          </header>
