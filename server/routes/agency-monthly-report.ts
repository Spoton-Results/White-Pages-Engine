...
async function ensureReportLinksTable() {
  await pool.query(`CREATE TABLE IF NOT EXISTS client_report_links (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), account_id TEXT NOT NULL, token TEXT NOT NULL UNIQUE, report_type TEXT NOT NULL DEFAULT 'monthly_visibility', expires_at TIMESTAMP, revoked_at TIMESTAMP, created_by TEXT, created_at TIMESTAMP DEFAULT NOW(), last_viewed_at TIMESTAMP, view_count INTEGER DEFAULT 0)`);
...
async function assertClientAccess(req: any, res: any, accountId: string) {
  ...
  if (!req.session.isSuperAdmin && String(req.session.accountId) !== String(accountId)) { res.status(403).send("Forbidden"); return null; }
  return account;
}

async function assertLinkAccess(req: any, res: any, linkId: string) {
  await ensureReportLinksTable();
  const result = await pool.query(`SELECT crl.*, a.name AS account_name FROM client_report_links crl JOIN accounts a ON a.id::text = crl.account_id::text WHERE crl.id::text = $1::text LIMIT 1`, [linkId]);
  const link = result.rows[0];
  if (!link) { res.status(404).json({ message: "Report link not found" }); return null; }
  if (!req.session.isSuperAdmin && String(req.session.accountId) !== String(link.account_id)) { res.status(403).json({ message: "Forbidden" }); return null; }
  return link;
}
...
async function renderMonthlyReport(accountId: string, options: { publicView?: boolean } = {}) {
  const accountResult = await pool.query(`SELECT id, name, status FROM accounts WHERE id::text = $1::text LIMIT 1`, [accountId]);
  const account = accountResult.rows[0];
  if (!account) return null;

  const [summary, pageTypes, topCities, topServices, workLog, websites] = await Promise.all([
    pool.query(`SELECT
      (SELECT COUNT(*)::int FROM pages p JOIN websites w ON w.id = p.website_id WHERE w.account_id::text = $1::text AND p.status = 'published') AS pages_live,
      (SELECT COUNT(DISTINCT slug)::int FROM locations WHERE account_id::text = $1::text AND type = 'city') AS cities_covered,
      (SELECT COUNT(DISTINCT slug)::int FROM services WHERE account_id::text = $1::text) AS services_covered,
      (SELECT COUNT(*)::int FROM pages p JOIN websites w ON w.id = p.website_id WHERE w.account_id::text = $1::text AND p.status = 'published' AND p.created_at >= NOW() - INTERVAL '30 days') AS pages_30d,
      (SELECT COUNT(*)::int FROM internal_links il JOIN websites w ON w.id = il.website_id WHERE w.account_id::text = $1::text AND il.created_at >= NOW() - INTERVAL '30 days') AS links_30d,
      (SELECT COUNT(*)::int FROM generation_jobs WHERE account_id::text = $1::text AND settings->>'type' = 'intent_page_improvement' AND created_at >= NOW() - INTERVAL '30 days') AS improvements_30d,
      (SELECT COUNT(*)::int FROM sitemaps sm JOIN websites w ON w.id = sm.website_id WHERE w.account_id::text = $1::text AND sm.updated_at >= NOW() - INTERVAL '30 days') AS sitemap_updates_30d,
      ...
      (SELECT COUNT(*)::int FROM variation_bank_completeness vbc JOIN websites w ON w.id = vbc.website_id WHERE w.account_id::text = $1::text AND vbc.completeness_score < 70) AS thin_banks`, [accountId]),
    pool.query(`SELECT p.page_type, COUNT(*)::int AS count FROM pages p JOIN websites w ON w.id = p.website_id WHERE w.account_id::text = $1::text AND p.status = 'published' GROUP BY p.page_type ORDER BY count DESC`, [accountId]),
    pool.query(`SELECT name, state_code, population FROM locations WHERE account_id::text = $1::text AND type = 'city' ORDER BY COALESCE(population,0) DESC LIMIT 8`, [accountId]),
    pool.query(`SELECT s.name, COUNT(p.id)::int AS pages_live FROM services s LEFT JOIN pages p ON p.service_id = s.id AND p.status = 'published' WHERE s.account_id::text = $1::text GROUP BY s.name ORDER BY pages_live DESC, s.name ASC LIMIT 8`, [accountId]),
    pool.query(`SELECT 'Page created' AS type, p.title AS label, p.slug AS detail, p.created_at FROM pages p JOIN websites w ON w.id = p.website_id WHERE w.account_id::text = $1::text AND p.created_at >= NOW() - INTERVAL '30 days' UNION ALL SELECT 'Job processed' AS type, gj.name AS label, gj.status::text AS detail, gj.created_at FROM generation_jobs gj WHERE gj.account_id::text = $1::text AND gj.created_at >= NOW() - INTERVAL '30 days' UNION ALL SELECT 'Internal link added' AS type, 'Internal link added' AS label, il.link_type AS detail, il.created_at FROM internal_links il JOIN websites w ON w.id = il.website_id WHERE w.account_id::text = $1::text AND il.created_at >= NOW() - INTERVAL '30 days' ORDER BY created_at DESC LIMIT 20`, [accountId]),
    pool.query(`SELECT name, domain, status FROM websites WHERE account_id::text = $1::text ORDER BY created_at DESC`, [accountId]),
  ]);
