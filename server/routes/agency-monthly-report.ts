...
async function assertClientAccess(req: any, res: any, accountId: string) {
  ...
  if (!req.session.isSuperAdmin && req.session.accountId && String(req.session.accountId) !== String(accountId)) { res.status(403).send("Forbidden"); return null; }
  return account;
}
...
async function assertLinkAccess(req: any, res: any, linkId: string) {
  ...
  if (!req.session.isSuperAdmin && req.session.accountId && String(req.session.accountId) !== String(link.account_id)) { res.status(403).json({ message: "Forbidden" }); return null; }
  return link;
}
...
async function renderMonthlyReport(accountId: string, options: { publicView?: boolean } = {}) {
  ...
  const [summary, pageTypes, topCities, topServices, workLog, websites] = await Promise.all([
    pool.query(`SELECT
      ...
      (SELECT COUNT(*)::int FROM internal_links il JOIN websites w ON w.id::text = il.website_id::text WHERE w.account_id::text = $1::text AND il.created_at >= NOW() - INTERVAL '30 days') AS links_30d,
      ...
      (SELECT COUNT(*)::int FROM sitemaps sm JOIN websites w ON w.id::text = sm.website_id::text WHERE w.account_id::text = $1::text AND sm.updated_at >= NOW() - INTERVAL '30 days') AS sitemap_updates_30d,
    ...
    pool.query(`SELECT s.name, COUNT(p.id)::int AS pages_live FROM services s LEFT JOIN pages p ON p.service_id::text = s.id::text AND p.status = 'published' WHERE s.account_id::text = $1::text GROUP BY s.name ORDER BY pages_live DESC, s.name ASC LIMIT 8`, [accountId]),
