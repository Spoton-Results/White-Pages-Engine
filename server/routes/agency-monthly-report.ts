...
async function assertLinkAccess(req: any, res: any, linkId: string) {
  ...
  const result = await pool.query(`SELECT crl.*, a.name AS account_name FROM client_report_links crl JOIN accounts a ON a.id = crl.account_id WHERE crl.id = $1 LIMIT 1`, [linkId]);
