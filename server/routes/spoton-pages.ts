/**
 * spoton-pages.ts
 * SpotOn-branded page serving — slug resolution and public-facing
 * page rendering for spotonresults.com / pages.spotonresults.com.
 * Graduated from spoton-pages-hotfix.ts (logic unchanged).
 */
...
router.all("/api/spoton-pages-repair-copy", async (_req: Request, res: Response, next: NextFunction) => {
  try {
    const versionUpdate = await pool.query(
      `WITH spoton_websites AS (
       ...
       SET content_html = regexp_replace(
         regexp_replace(
           regexp_replace(
             regexp_replace(pv.content_html,
               'free equipment\\s*&\\s*fast setup for\\s*\\.\\s*Get a free quote today\\.',
               'free equipment & fast setup for local businesses. Get a free quote today.', 'gi'),
    ...
    return res.json({
      ok: true,
      pageVersionsRepaired: versionUpdate.rowCount || 0,
      pagesMarkedForRerender: pageUpdate.rowCount || 0,
      sample: pageUpdate.rows.slice(0, 10),
    });
  } catch (err) {
    return next(err);
  }
...
router.get("/api/spoton-pages-debug/:slug", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const websites = await pool.query(
      `SELECT id, domain,
              settings->>'parentDomain' AS parent_domain,
              settings->>'publicDomain' AS public_domain,
              settings->>'legacyParentDomain' AS legacy_parent_domain,
              updated_at
       FROM websites
       WHERE lower(domain) IN ($1, $2)
          OR lower(settings->>'parentDomain') IN ($1, $2)
    ...
    return res.json({
      host,
      slug,
      spotonWebsiteCount: websites.rowCount,
      spotonWebsites: websites.rows,
...
// ── Public page serving middleware ────────────────────────────────────────────
router.use(async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const content = sanitizeSpotOnCopy(
      version.content_html || version.contentHtml ||
      page.content_html || page.contentHtml ||
      page.html || page.body || ""
    );
