...
router.get("/api/websites/:websiteId/production-validation", requireWebsiteParam, async (req, res, next) => {
  try {
    ...
    const [pages, services, bankRows, jobs, reviewJobs, internalLinks, sitemaps] = await Promise.all([
      pool.query(`SELECT COUNT(*)::int AS count FROM pages WHERE website_id = $1`, [websiteId]),
      pool.query(`SELECT COUNT(*)::int AS count FROM services WHERE account_id = $1`, [(req as any).website.account_id]),
      pool.query(`SELECT COUNT(*)::int AS count, COALESCE(AVG(completeness_score),0)::float AS avg_score FROM variation_bank_completeness WHERE website_id = $1`, [websiteId]),
      pool.query(`SELECT status, COUNT(*)::int AS count FROM generation_jobs WHERE website_id = $1 GROUP BY status`, [websiteId]),
      pool.query(`SELECT COUNT(*)::int AS count FROM generation_jobs WHERE website_id = $1 AND settings->>'type' IN ('intent_consolidation_review','intent_merge_review')`, [websiteId]),
      pool.query(`SELECT COUNT(*)::int AS count FROM internal_links WHERE website_id = $1`, [websiteId]),
      pool.query(`SELECT COUNT(*)::int AS count FROM sitemaps WHERE website_id = $1`, [websiteId]),
    ]);
    ...
    res.json({
      ...
      summary: { publishedPages, serviceCount, bankCount, avgBankScore, linkCount, sitemapCount, failedJobs, pendingJobs },
      checks,
    });
  } catch (err) { next(err); }
});
  ...
  try {
    const result = await pool.query(
      `SELECT gj.id, gj.name, gj.status, gj.settings, gj.created_at, gj.completed_at, gj.error_log,
       ...
       WHERE gj.website_id = $1 AND gj.settings->>'type' IN ('intent_consolidation_review', 'intent_merge_review')
...
router.post("/api/action-review/:jobId/decision", async (req, res, next) => {
  try {
    ...
    const job = (await pool.query(`SELECT id, website_id, settings, error_log FROM generation_jobs WHERE id = $1 LIMIT 1`, [req.params.jobId])).rows[0];
    ...
    await pool.query(`UPDATE generation_jobs SET settings = jsonb_set(COALESCE(settings, '{}'::jsonb), '{reviewDecision}', $2::jsonb, true), error_log = $3::jsonb, updated_at = NOW() WHERE id = $1`, [req.params.jobId, JSON.stringify(decision), JSON.stringify(notes)]);
