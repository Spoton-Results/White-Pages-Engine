  ...
  try {
    ...
    const result = await pool.query(
      `WITH page_counts AS (
         ...
         SELECT w.account_id, COUNT(vbc.website_id)::int AS thin_banks FROM websites w LEFT JOIN variation_bank_completeness vbc ON vbc.website_id = w.id AND vbc.completeness_score < 70 GROUP BY w.account_id
       )
