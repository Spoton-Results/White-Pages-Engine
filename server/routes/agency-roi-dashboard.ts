...
router.get("/api/agency-dashboard/summary", requireAuth, async (req, res, next) => {
  try {
    ...
    const [clients, pages, pagesBuiltThisMonthRows, clientsWithNewWorkRows, cities, services, failedJobsRows, thinBankRows] = await Promise.all([
      ...
      // ✅ CHANGED: Avoid full pages-table scan on executive summary; use website counter maintained after generation.
      // 🔒 UNTOUCHED: Metric response shape and remaining dashboard endpoints stay unchanged.
      pool.query(`SELECT COALESCE(SUM(w.published_pages), 0)::int AS count FROM websites w JOIN accounts a ON a.id::text = w.account_id::text ${scope.where}`, scope.params),
