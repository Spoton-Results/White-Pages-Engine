...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  //
  // IMPORTANT: Use req.originalUrl (not req.path) so the full path is checked.
  // req.path can be stripped of prefixes by sub-routers, causing session MW to
  // be skipped for routes like /api/auth/login → "Session middleware not initialized".
  const _sessionMW = sessionMiddleware();
  app.use((req: Request, res: Response, next: NextFunction) => {
    const url = req.originalUrl || req.url || "";
    if (!url.startsWith("/api/") && !url.startsWith("/api")) return next();
