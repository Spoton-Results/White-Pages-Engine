...
healthScore is 0-100. priority must be "critical", "important", or "nice-to-have". Set done=true for items that are already set up based on the state above.`,
  ...
  // TEMP: one-shot Cloudflare Worker deployment for subtrackers.spotonresults.com
  app.post("/api/admin/deploy-subtrackers-worker", async (req: Request, res: Response) => {
    const cfToken = process.env.CLOUDFLARE_API_TOKEN;
    const zoneId = "8fc5544bff85fbfe1b8580a0e75c1ede";
    const accountId = "d50cb9c8e028ae248d2bf9f057053a0b";
    const workerName = "subtrackers-proxy";
    if (!cfToken) return res.status(500).json({ error: "CLOUDFLARE_API_TOKEN not set" });
    const log: string[] = [];
    try {
      // 1. Upload worker script
      const { readFileSync } = await import("fs");
      const { join } = await import("path");
      const scriptPath = join(process.cwd(), "workers/subtrackers-proxy.js");
      const script = readFileSync(scriptPath, "utf8");
      const form = new FormData();
      form.append("metadata", JSON.stringify({ main_module: "worker.js", bindings: [{ type: "plain_text", name: "NEXUS_ORIGIN", text: "https://sospages.replit.app" }] }));
      form.append("worker.js", new Blob([script], { type: "application/javascript+module" }), "worker.js");
      const uploadResp = await fetch(`https://api.cloudflare.com/client/v4/accounts/${accountId}/workers/scripts/${workerName}`, {
        method: "PUT", headers: { "Authorization": `Bearer ${cfToken}` }, body: form,
      });
      const uploadJson: any = await uploadResp.json();
      log.push(`worker upload: success=${uploadJson.success} ${JSON.stringify(uploadJson.errors)}`);
      if (!uploadJson.success) return res.json({ ok: false, log });

      // 2. Check/create DNS record for subtrackers.spotonresults.com (proxied, points to placeholder)
      const dnsListResp = await fetch(`https://api.cloudflare.com/client/v4/zones/${zoneId}/dns_records?name=subtrackers.spotonresults.com`, {
        headers: { "Authorization": `Bearer ${cfToken}` }
      });
      const dnsList: any = await dnsListResp.json();
      log.push(`existing DNS records: ${dnsList.result?.length || 0}`);
      if (!dnsList.result?.length) {
        const dnsCreate = await fetch(`https://api.cloudflare.com/client/v4/zones/${zoneId}/dns_records`, {
          method: "POST", headers: { "Authorization": `Bearer ${cfToken}`, "Content-Type": "application/json" },
          body: JSON.stringify({ type: "AAAA", name: "subtrackers.spotonresults.com", content: "100::", proxied: true, ttl: 1 })
        });
        const dnsJson: any = await dnsCreate.json();
        log.push(`DNS create: success=${dnsJson.success} ${JSON.stringify(dnsJson.errors)}`);
      } else {
        log.push("DNS record already exists");
      }

      // 3. Create worker route
      const routeResp = await fetch(`https://api.cloudflare.com/client/v4/zones/${zoneId}/workers/routes`, {
        method: "POST", headers: { "Authorization": `Bearer ${cfToken}`, "Content-Type": "application/json" },
        body: JSON.stringify({ pattern: "subtrackers.spotonresults.com/*", script: workerName })
      });
      const routeJson: any = await routeResp.json();
      log.push(`route create: success=${routeJson.success} ${JSON.stringify(routeJson.errors)}`);
