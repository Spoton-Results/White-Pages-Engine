...
export async function getDashboardStats() {
  ...
  const { pool } = await import("./db");
  // ✅ CHANGED: added totalBrandProfiles and totalIndustries counts so the
  // Overview dashboard KPIs resolve correctly without falling back to the
  // account-scoped /api/brand-profiles and /api/industries endpoints (which
  // require an accountId param and return empty arrays without one).
  const [accountRes, websiteRes, jobRes, brandRes, industryRes] = await Promise.all([
    ...
    pool.query(`SELECT COUNT(*)::int AS count FROM brand_profiles`), // ✅ CHANGED
    pool.query(`SELECT COUNT(*)::int AS count FROM industries`),     // ✅ CHANGED
  ]);
  ...
  return {
    ...
    totalBrandProfiles: Number(brandRes.rows[0]?.count ?? 0),   // ✅ CHANGED
    totalIndustries:    Number(industryRes.rows[0]?.count ?? 0), // ✅ CHANGED
    draftPages:         0,
    reviewPages:        0,
