...
export async function getDashboardStats() {
  ...
  const { pool } = await import("./db");
  const [accountRes, websiteRes, jobRes, brandProfileRes, industryRes] = await Promise.all([
  ...
  return {
    ...
    totalBrandProfiles: Number(brandProfileRes.rows[0]?.count ?? 0),
    totalIndustries: Number(industryRes.rows[0]?.count ?? 0),
  };
}
