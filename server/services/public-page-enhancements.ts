...
function sanitizePublicCopy(value: unknown): string {
  return String(value || "")
    ...
    .replace(/\b(?:[A-Za-z0-9&'’-]+)\s+s team\b/g, "the team")
    .replace(/\bs team\b/gi, "the team")
    .replace(/\s+([,.;:!?])/g, "$1")
    .replace(/\s{2,}/g, " ")
    .trim();
}
...
function enhanceContentSections(html: string): string {
  ...
  return html.replace(headingPattern, (block, headingHtml, bodyHtml) => {
    ...
    if (
      ...
      /frequently asked questions/.test(headingText) ||
      /^faq/.test(headingText)
    ) {
      type = "faq";
    } else if (
      /cost and pricing/.test(headingText) ||
      /pricing factors/.test(headingText) ||
      /what .* costs/.test(headingText)
    ) {
      type = "pricing";
    } else if (
      /use cases/.test(headingText) ||
      /case stud/.test(headingText)
    ) {
      type = "use-cases";
    } else if (
      /trust and service confidence/.test(headingText) ||
      /why trust/.test(headingText) ||
      /experience and credentials/.test(headingText)
    ) {
      type = "trust";
    } else if (
      /market context/.test(headingText) ||
      /market signals/.test(headingText)
    ) {
      type = "market";
    } else if (
      /common .* problems/.test(headingText) ||
      /common challenges/.test(headingText)
    ) {
      type = "problems";
    } else if (
      /software and integration/.test(headingText) ||
      /integrations/.test(headingText)
    ) {
      type = "integrations";
    } else if (
      /related services/.test(headingText)
    ) {
...
function testimonials(website: any) {
  const settings = website?.settings || {};
  const saved = Array.isArray(settings.testimonials)
  ...
  return `<section class="nexus-card nexus-testimonials"><p class="nexus-eyebrow">Customer feedback</p><h2>What Customers Say</h2><div class="nexus-testimonial-grid">${items.map((testimonial: any) => `<blockquote class="nexus-testimonial"><p>“${escapeHtml(testimonial.quote)}”</p>${testimonial.name || testimonial.title ? `<footer><strong>${escapeHtml(testimonial.name)}</strong>${testimonial.title ? `<span>${escapeHtml(testimonial.title)}</span>` : ""}</footer>` : ""}</blockquote>`).join("")}</div></section>`;
}
